...
const hsCompletion = require("../../providers/hs_module_completion");
 ...
 */
function compare(module, provider, isMethod = false) {
    let n = 0;
...
suite("HS Completion", () => {
    test("No suggestions", async () => {
        ...
        const provider = await completion.provideCompletionItems(editor.document, new vscode.Position(0, 3));
        assert.ok(!provider);
    });
    ...
    test("Completion for: `hs.applc`", async () => {
        ...
        const completion = new hsCompletion.HSModulesCompletionProvider();
        const provider = await completion.provideCompletionItems(editor.document, new vscode.Position(0, 9));
        assert.ok(provider);
        compare("hs", provider);
    });
    test("Completion for: `hs.application('Code):`", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile, 1);
        const completion = new hsCompletion.HSModulesCompletionProvider();
        const provider = await completion.provideCompletionItems(editor.document, new vscode.Position(0, 24));
        assert.ok(provider);
    ...
    test("Completion for: `app[1]:`", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const completion = new hsCompletion.HSModulesCompletionProvider();
        const provider = await completion.provideCompletionItems(editor.document, new vscode.Position(1, 23));
        assert.ok(provider);
        compare("hs.application", provider, true);
    });
    test("Completion for: `app.foo:`", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const completion = new hsCompletion.HSModulesCompletionProvider();
        const provider = await completion.provideCompletionItems(editor.document, new vscode.Position(1, 24));
        assert.ok(provider);
        compare("hs.application", provider, true);
    });
    test("Completion for: `app:mainWindow():", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const completion = new hsCompletion.HSModulesCompletionProvider();
        const provider = await completion.provideCompletionItems(editor.document, new vscode.Position(1, 33));
        assert.ok(provider);
    ...
    test("Completion for: `app:` 1", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const completion = new hsCompletion.HSModulesCompletionProvider();
        const provider = await completion.provideCompletionItems(editor.document, new vscode.Position(1, 19));
        assert.ok(provider);
        compare("hs.application", provider, true);
    });
    ...
    test("Completion for: `myapp:` from identifier", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const completion = new hsCompletion.HSModulesCompletionProvider();
        const provider = await completion.provideCompletionItems(editor.document, new vscode.Position(2, 21));
        assert.ok(provider);
        compare("hs.application", provider, true);
    });
    test("Completion for `app:` below a if statement", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const completion = new hsCompletion.HSModulesCompletionProvider();
        const provider = await completion.provideCompletionItems(editor.document, new vscode.Position(2, 19));
        assert.ok(provider);
        compare("hs.application", provider, true);
    });
    test("Completion for if nested `app:`", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const completion = new hsCompletion.HSModulesCompletionProvider();
        const provider = await completion.provideCompletionItems(editor.document, new vscode.Position(3, 19));
        assert.ok(provider);
    ...
    test("Completion for multi level nested `app:`", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const completion = new hsCompletion.HSModulesCompletionProvider();
        const provider = await completion.provideCompletionItems(editor.document, new vscode.Position(6, 28));
        assert.ok(provider);
        compare("hs.chooser", provider, true);
    });
    test("Completion for: `tab.foo.bar:` from expression", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const completion = new hsCompletion.HSModulesCompletionProvider();
        const provider = await completion.provideCompletionItems(editor.document, new vscode.Position(5, 27));
        assert.ok(provider);
        compare("hs.application", provider, true);
    });
    test("Completion for: `tab.foo.bar:` from identifier", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const completion = new hsCompletion.HSModulesCompletionProvider();
        const provider = await completion.provideCompletionItems(editor.document, new vscode.Position(2, 27));
        assert.ok(provider);
        compare("hs.application", provider, true);
    });
    test("Completion for: `tab[1][1]:` from expression", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const completion = new hsCompletion.HSModulesCompletionProvider();
        const provider = await completion.provideCompletionItems(editor.document, new vscode.Position(5, 25));
        assert.ok(provider);
        compare("hs.application", provider, true);
    });
    test("Completion for: `tab[1][1]:` from identifier", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const completion = new hsCompletion.HSModulesCompletionProvider();
        const provider = await completion.provideCompletionItems(editor.document, new vscode.Position(2, 34));
        assert.ok(provider);
        compare("hs.application", provider, true);
    });
});
//# sourceMappingURL=hs_completion.test.js.map