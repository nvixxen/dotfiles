...
const hsHelper = require("../../providers/hs_helper");
...
suite("Hs Helper", () => {
    test("Skip if no word", async () => {
        ...
        const provider = await helper.provideSignatureHelp(editor.document, new vscode.Position(0, 5));
        assert.ok(!provider);
    });
    test("Helper active parameter", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const helper = new hsHelper.HsSignatureHelpProvider();
        const provider = await helper.provideSignatureHelp(editor.document, new vscode.Position(0, 36));
        assert.ok(provider);
    ...
    test("Helper for `activate` on `hs.application():activate()`", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const helper = new hsHelper.HsSignatureHelpProvider();
        const provider = await helper.provideSignatureHelp(editor.document, new vscode.Position(0, 26));
        assert.ok(provider);
    ...
    test("Helper for `setSize` on `app:mainWindow():setSize()`", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const helper = new hsHelper.HsSignatureHelpProvider();
        const provider = await helper.provideSignatureHelp(editor.document, new vscode.Position(1, 40));
        assert.ok(provider);
        ...
        assert.strictEqual(provider.signatures[0].parameters[0].documentation, "A size-table containing the width and height the window should be resized to");
    });
    test("Helper for `isRunning` on `app:isRunning(`", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const helper = new hsHelper.HsSignatureHelpProvider();
        const provider = await helper.provideSignatureHelp(editor.document, new vscode.Position(1, 29));
        assert.ok(provider);
    ...
    test("Helper for `activate` on `app:activate(`", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const helper = new hsHelper.HsSignatureHelpProvider();
        const provider = await helper.provideSignatureHelp(editor.document, new vscode.Position(1, 33));
        assert.ok(provider);
        ...
        assert.strictEqual(provider.signatures[0].parameters[0].documentation, "the desired window's title string as per `hs.window:title()`");
    });
    test("Helper for `activate` on `app[1]:activate(`", async () => {
        ...
        const editor = await testUtils.focusDemoFile(demoFile);
        const helper = new hsHelper.HsSignatureHelpProvider();
        const provider = await helper.provideSignatureHelp(editor.document, new vscode.Position(1, 32));
        assert.ok(provider);
        assert.strictEqual(provider.signatures[0].label, "(title) -> hs.window object");
        assert.strictEqual(provider.signatures[0].parameters[0].label, "title");
        assert.strictEqual(provider.signatures[0].parameters[0].documentation, "the desired window's title string as per `hs.window:title()`");
...
//# sourceMappingURL=hs_helper.test.js.map