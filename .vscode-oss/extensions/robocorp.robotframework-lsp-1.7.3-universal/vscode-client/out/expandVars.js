...
function expandVars(template) {
    try {
        if (typeof template !== "string") {
            channel_1.OUTPUT_CHANNEL.appendLine("Converting " + template + "(type:" + typeof template + ") to string to expand variables.");
            template = "" + template;
        }
        ...
        (0, channel_1.logError)(msg, err, "ERROR_EXPANDING_VARIABLE");
        throw new Error(msg);
    }
}
...
function getArrayStrFromConfigExpandingVars(config, name) {
    let arrayOrStr = config.get(name);
    let array;
